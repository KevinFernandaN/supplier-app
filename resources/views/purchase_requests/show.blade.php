    ...
    <div class="col-md-4">
        <div class="card h-100">
            <div class="card-body">
                ...
                <div class="fw-semibold">{{ $purchaseRequest->menu->name ?? 'All Menus (Period)' }}</div>
            </div>
        </div>
    </div>
</div>

@if ($purchaseRequest->rabPeriod)
    <div class="alert alert-light border mb-4 d-flex justify-content-between align-items-center">
        <div>
            Generated from RAB Period
            <a href="{{ route('rab-periods.show', $purchaseRequest->rabPeriod) }}">{{ $purchaseRequest->rabPeriod->name }}</a>
            &mdash;
            {{ $purchaseRequest->rabPeriod->start_date->format('d M Y') }}
            &rarr;
            {{ $purchaseRequest->rabPeriod->end_date->format('d M Y') }}
        </div>
    </div>
@endif
